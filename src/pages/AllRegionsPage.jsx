...
function AllRegionsPage() {
    ...
    useEffect(() => {
        axios
            ...
            if (isLoggedIn && user) {
                axios
                    .get(`${import.meta.env.VITE_API_URL}/api/users/${user._id}/favoritesRegions`)
                    .then((response) => {
                        console.log("fetched")
    ...
    const removeFavorite = (regionId) => {
        axios.delete(`${import.meta.env.VITE_API_URL}/api/regions/${regionId}/favorites/${user._id}`)
        .then((response) => {
            console.log(response);
    ...
    return (
        <div>
            ...
            <div style={{ marginBottom: '20px', textAlign: 'center' }}>
                ...
                <Input placeholder="Search region..." onChange={handleSearch} style={{ width: 200 }} />
            </div>
            <div
                style={{
                    display: "flex",
                    ...
                    >
                        ...
                        >
                            ...
                            <Flex gap="small" wrap="wrap">
                                {isLoggedIn && (
                                    <button onClick={() => toggleFavorite(region._id)} style={{ background: 'none', border: 'none', cursor: 'pointer', outline: 'none' }}>
    {favoritesRegions.includes(region._id) ? <HeartFilled style={{ color: '#5F4E44' }} /> : <HeartOutlined />}
</button>
