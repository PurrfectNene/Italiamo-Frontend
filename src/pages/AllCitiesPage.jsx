        ...
        axios
            ...
            .catch((err) => {
                console.log(err);
                        ...
                        if (isLoggedIn && user) {
                axios
                    .get(`${import.meta.env.VITE_API_URL}/api/users/${user._id}/favoritesCities`)
                    .then((response) => {
                        console.log("fetched")
...
useEffect(() => {
    ...
    const addFavorite = (cityId) => {
        axios.post(`${import.meta.env.VITE_API_URL}/api/cities/${cityId}/favorites`, {
            userId: user._id,
        })
        .then((response) => {
    ...
    const removeFavorite = (cityId) => {
        axios.delete(`${import.meta.env.VITE_API_URL}/api/cities/${cityId}/favorites/${user._id}`)
        .then((response) => {
    ...
    return (
      <div>
          ...
          <div style={{ marginBottom: '20px', textAlign: 'center' }}>
              ...
              <Input placeholder="Search by region..." onChange={handleSearch} style={{ width: 200 }} />
          </div>
          <div
              style={{
                  display: "flex",
              ...
              {filteredCities.map((city) => {
                  return (
                      ...
                      >
                          ...
                          >
                              {isLoggedIn && (
                                    <button onClick={() => toggleFavorite(city._id)} style={{ background: 'none', border: 'none', cursor: 'pointer', outline: 'none' }}>
                                    {favoritesCities.includes(city._id) ? <HeartFilled style={{ color: '#5F4E44' }} /> : <HeartOutlined />}
                                </button>
                              )}
                              <Link
                                  to={`/cities/${city._id}`}
